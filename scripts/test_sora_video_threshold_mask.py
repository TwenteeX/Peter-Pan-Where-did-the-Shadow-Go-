"""
Experimental Sora video -> threshold-mask sprite frames.

This is intentionally separate from peter_pan.generation.sprite_cache so the
current image sprite-sheet path stays untouched.

Default run creates a tiny synthetic video and tests the threshold-masking
pipeline locally:

  python -m scripts.test_sora_video_threshold_mask

To call the Sora Video API, set OPENAI_API_KEY and pass --generate-sora:

  python -m scripts.test_sora_video_threshold_mask --generate-sora
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Test Sora video generation followed by threshold-mask extraction."
    )
    parser.add_argument(
        "--generate-sora",
        action="store_true",
        help="Call OpenAI Sora Video API. Without this, run a local synthetic test.",
    )
    parser.add_argument(
        "--video-path",
        type=Path,
        default=None,
        help="Process an existing video instead of generating one.",
    )
    parser.add_argument(
        "--prompt",
        default=None,
        help="Override the generated Sora prompt.",
    )
    parser.add_argument(
        "--prompt-file",
        type=Path,
        default=None,
        help="Read the Sora prompt from a UTF-8 text file.",
    )
    parser.add_argument(
        "--character",
        default=DEFAULT_CHARACTER,
        help="Character name for the default prompt, e.g. cat, teddy bear, fox.",
    )
    parser.add_argument(
        "--action",
        default=DEFAULT_ACTION,
        help="Action for the default prompt, e.g. walking, idle breathing, hopping.",
    )
    parser.add_argument(
        "--travel-across-frame",
        action="store_true",
        help="Ask the character to walk left-to-right inside the video. Off by default because plane motion already moves the sprite.",
    )
    parser.add_argument(
        "--print-prompt",
        action="store_true",
        help="Print the final Sora prompt before running.",
    )
    parser.add_argument("--model", default="sora-2", choices=["sora-2", "sora-2-pro"])
    parser.add_argument("--seconds", default="4", choices=["4", "8", "12"])
    parser.add_argument(
        "--size",
        default="1280x720",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional reference image. It should match the target video size.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("assets/generated_sora_masks"),
    )
    parser.add_argument(
        "--threshold",
        type=int,
        default=170,
        help="Grayscale threshold. For black-on-white Sora prompts, dark pixels become foreground.",
    )
    parser.add_argument(
        "--foreground",
        default="dark",
        choices=["dark", "light"],
        help="Which side of the threshold is treated as foreground.",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Keep only the largest threshold component per frame.",
    )
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    prompt = resolve_prompt(args)
    write_prompt_file(args.output_dir / "sora_prompt.txt", prompt)
    ...
    if args.video_path is not None:
        ...
        video_path = create_synthetic_shadow_video(args.output_dir / "synthetic_source.mp4")

    result = extract_threshold_mask_frames(
        video_path,
        args.output_dir / video_path.stem,
...
def generate_sora_video(
    *,
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise SystemExit("OPENAI_API_KEY is not set; refusing to call Sora.")
    ...
    if not hasattr(client, "videos"):
        raise SystemExit("This openai package does not expose client.videos; upgrade openai.")
...
def _assert_usable_result(result: ThresholdMaskResult) -> None:
